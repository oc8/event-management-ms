...
async fn create_basic_event() -> Result<(), Box<dyn std::error::Error>> {
    let ctx = TestContext::new("postgres://postgres:postgres@localhost:5433", "create_basic_event", "redis://:@localhost:6382", 6070);
...
async fn get_event_not_found() -> Result<(), Box<dyn std::error::Error>> {
    let ctx = TestContext::new("postgres://postgres:postgres@localhost:5433", "get_event_not_found", "redis://:@localhost:6382", 6071);
    let (tx, rx) = oneshot::channel();
    let service = ctx.service.clone();
...
async fn get_event() -> Result<(), Box<dyn std::error::Error>> {
    let ctx = TestContext::new("postgres://postgres:postgres@localhost:5433", "get_event", "redis://:@localhost:6382", 6072);
    let (tx, rx) = oneshot::channel();
    let service = ctx.service.clone();
