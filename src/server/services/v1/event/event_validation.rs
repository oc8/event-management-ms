...
impl ValidateRequest for CreateEventRequest {
    fn validate(&self) -> Result<(), ApiError> {
        ...
        let start = DateTime::parse_from_rfc3339(&self.start).map_err(|_| {
        ...
        let end = DateTime::parse_from_rfc3339(&self.end).map_err(|_| {
            errors.push(ValidationErrorKind::InvalidFormat(
                "end".to_string(),
                ValidationErrorMessage::InvalidDateTime(),
            ))
        ...
        if !validate_recurrence_rule(self.recurrence_rule.as_str()) {
            errors.push(ValidationErrorKind::InvalidFormat(
                ...
                ValidationErrorMessage::InvalidRecurrenceRule(),
            ))
        }

        match EventType::try_from(self.event_type) {
            Ok(et) => match et {
                EventType::Unspecified => errors.push(ValidationErrorKind::InvalidType(
                    "event_type".to_string(),
                    EventType::Unspecified.as_str_name().to_string(),
                )),
                EventType::Meeting => {
                    if !self.capacity.validate_range(
                        Option::from(1),
                        Option::from(10000),
                        Option::from(1),
                        Option::from(10000),
                    ) && !self.slot_capacity.validate_range(
                        Option::from(1),
                        Option::from(10000),
                        Option::from(1),
                        Option::from(10000),
                    ) {
                        errors.push(ValidationErrorKind::InvalidRange(
                            "capacity, slot_capacity".to_string(),
                            1,
                            10000,
                        ))
                    }

                    if !self.slot_duration.validate_range(
                        Option::from(1),
                        Option::from(1440),
                        Option::from(1),
                        Option::from(1440),
            ...
            Err(_) => errors.push(ValidationErrorKind::InvalidType(
                "event_type".to_string(),
                "".to_string(),
            )),
        }
...
impl ValidateRequest for UpdateEventRequest {
    fn validate(&self) -> Result<(), ApiError> {
        ...
        if !self.start.is_empty() {
            let _ = DateTime::parse_from_rfc3339(&self.start).map_err(|_| {
                errors.push(ValidationErrorKind::InvalidFormat(
                    "start".to_string(),
                    ValidationErrorMessage::InvalidDateTime(),
                ))
        ...
        if !self.end.is_empty() {
            let _ = DateTime::parse_from_rfc3339(&self.end).map_err(|_| {
                errors.push(ValidationErrorKind::InvalidFormat(
                    "end".to_string(),
                    ValidationErrorMessage::InvalidDateTime(),
                ))
...
impl ValidateRequest for CancelEventRequest {
    fn validate(&self) -> Result<(), ApiError> {
        ...
        if !self.reason.is_empty() && !self.reason.validate_length(Some(1), Some(500), None) {
            return Err(ValidationError(List(vec![
                ValidationErrorKind::InvalidLength("reason".to_string(), 1, 500),
            ])));
        }

        if !self.reason.is_empty() && !self.canceled_by.validate_length(Some(1), Some(100), None) {
            return Err(ValidationError(List(vec![
                ValidationErrorKind::InvalidLength("canceled_by".to_string(), 1, 100),
            ])));
        }
