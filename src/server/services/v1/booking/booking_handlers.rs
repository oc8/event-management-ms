...
pub async fn create_booking(
    ...
    match event.capacity {
        ...
        Some(capacity) => {
            ...
            let sum_persons = conn
                .sum_persons_by_event(event.id, start_date, end_date)
                .await?;
    ...
    let booking = conn
        .get_booking_holder_booking(slot_id, request.booking_holder_key.clone(), date_time)
        .await;
...
pub async fn get_booking_by_id(
    ...
    let booking = conn.get_booking_by_id(booking_id).await?;
...
pub async fn list_bookings(
    ...
    let bookings = conn.get_bookings_with_filters(&filters).await?;

    Ok(ListBookingsResponse {
        bookings: bookings
            .into_iter()
            .map(|booking| booking.to_response(get_meta_timezone(meta)))
            .collect(),
    })
}
...
pub async fn delete_booking(
    ...
    conn.delete_booking(booking_id).await?;
