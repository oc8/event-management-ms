...
impl ValidateRequest for CreateClosureRequest {
    fn validate(&self) -> Result<(), ApiError> {
        ...
        let start = DateTime::parse_from_rfc3339(&self.closing_from)
            .map_err(|_| {
                errors.push(ValidationErrorKind::InvalidFormat("closing_from".to_string(), ValidationErrorMessage::InvalidDateTime()))
            });

        let end = DateTime::parse_from_rfc3339(&self.closing_to)
            .map_err(|_| {
                errors.push(ValidationErrorKind::InvalidFormat("closing_to".to_string(), ValidationErrorMessage::InvalidDateTime()))
            });

        if start.is_ok() && end.is_ok() && (start.unwrap() >= end.unwrap()) {
            errors.push(ValidationErrorKind::InvalidDateRange("closing_from, closing_to".to_string(), "closing_from must be before closing_to".to_string()))
        }
...
impl ValidateRequest for UpdateClosureRequest {
    fn validate(&self) -> Result<(), ApiError> {
        ...
        if Uuid::parse_str(&self.id).is_err() {
            return Err(ValidationError(List::<ValidationErrorKind>(vec![ValidationErrorKind::InvalidFormat("id".to_string(), ValidationErrorMessage::InvalidUuid())])))
        }
