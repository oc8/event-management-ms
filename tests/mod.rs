...
impl TestContext {
    async fn new(db_url: &str, db_name: &str, r_url: &str, port: u16) -> Self {
        dotenvy::dotenv().ok();
        let pool = PgPool::connect(&format!("{}/postgres", db_url))
            .await
        ...
        sqlx::query(&query)
            .execute(&pool)
            .await
            .expect(&format!("Could not drop database {}", db_name));
        ...
        let pool = Arc::new(
            PgPool::connect(&format!("{}/{}", db_url, db_name))
                .await
                .expect("Cannot connect to new database"),
        );
        ...
        let r_client = Client::open(r_url).expect("Cannot connect to redis server");

        let cache_ttl = env::var("CACHE_TTL")
            .unwrap_or_else(|_| "60".to_string())
            .parse::<u64>()
            .expect("CACHE_TTL must be a number");
    ...
    async fn cleanup(&self) {
        let pool = PgPool::connect(&format!("{}/postgres", self.db_url))
            .await
            .expect("Cannot connect to postgres database.");
